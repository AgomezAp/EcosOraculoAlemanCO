  ...
  implements OnInit, OnDestroy, AfterViewChecked, AfterViewInit
{
  ...
  counselorInfo = {
    name: 'Dr. Valeria',
    title: 'Berufliche Orientierungsspezialistin',
    specialty: 'Professionelle Orientierung und personalisierte Berufskarten',
  };
  //Datos para enviar
          ...
          setTimeout(() => {
            this.addMessage({
              ...
              content:
                ...
                '✨ Vielen Dank für deine Zahlung. Du hast jetzt vollen Zugriff auf die Berufskarte.\n\n' +
                '💼 Lass uns gemeinsam deine berufliche Zukunft entdecken!\n\n' +
                '📌 Hinweis: Diese Zahlung gilt nur für den Berufskarte-Service. Für andere Dienste ist eine separate Zahlung erforderlich.',
              timestamp: new Date(),
              isUser: false,
            });
  ...
  initializeWelcomeMessage(): void {
    this.addMessage({
      sender: this.counselorInfo.name,
      content: `Hallo! Ich bin ${this.counselorInfo.name}, deine Berufliche Orientierungsspezialistin. Ich bin hier, um dir zu helfen, deine wahre Berufung zu entdecken und eine personalisierte Berufskarte zu entwerfen.`,
      timestamp: new Date(),
      isUser: false,
    });
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.addMessage({
            sender: this.counselorInfo.name,
            content:
              'Entschuldigung, ich habe technische Schwierigkeiten. Könntest du deine Frage umformulieren?',
            timestamp: new Date(),
            isUser: false,
          });
          this.saveMessagesToSession();
  ...
  onPrizeWon(prize: Prize): void {
    const prizeMessage: ChatMessage = {
      sender: this.counselorInfo.name,
      content: `🎯 Großartig! Das berufliche Schicksal hat dich gesegnet. Du hast gewonnen: **${prize.name}** ${prize.icon}\n\nDieses Geschenk des beruflichen Universums wurde für dich aktiviert. Berufliche Möglichkeiten richten sich zu deinen Gunsten aus. Möge dieses Glück dich zu deiner wahren Berufung führen!`,
      timestamp: new Date(),
      isUser: false,
    };
    ...
    switch (prize.id) {
      ...
      case '2': // 1 Análisis Premium - ACCESO COMPLETO
        ...
        const premiumMessage: ChatMessage = {
          sender: this.counselorInfo.name,
          content:
            '✨ **Du hast den vollständigen Premium-Zugang freigeschaltet!** ✨\n\nDas berufliche Schicksal hat dich auf außergewöhnliche Weise angelächelt. Jetzt hast du unbegrenzten Zugang zu all meiner Erfahrung in der Berufsberatung. Du kannst so oft wie du möchtest über deine Berufung, berufliche Bewertungen und alle Aspekte deiner beruflichen Zukunft konsultieren.\n\n🎯 *Die Türen deines beruflichen Weges haben sich vollständig geöffnet* 🎯',
          timestamp: new Date(),
          isUser: false,
        };
  ...
  async handlePaymentSubmit(): Promise<void> {
    ...
    try {
      ...
      const orderData = {
        ...
        returnPath: '/berufskarte',  // ✅ Ruta correcta en alemán
        cancelPath: '/berufskarte'   // ✅ Ruta si cancela el pago
      };
  ...
  savePersonalInfo(): void {
    ...
    if (Object.keys(this.personalInfo).length > 0) {
      this.addMessage({
        sender: this.counselorInfo.name,
        content: `Perfekt, ich habe deine persönlichen Informationen registriert. Dies wird mir helfen, dir eine genauere und personalisiertere Orientierung zu geben. Gibt es etwas Spezielles über deine berufliche Zukunft, das dich beunruhigt oder begeistert?`,
        timestamp: new Date(),
        isUser: false,
      });
    }
